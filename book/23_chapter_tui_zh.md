...
| Tool/file detail | 双击 tool card（已折叠的命令 / 读取 / 编辑卡片：meta 行末尾的 `双击查看`） | `popups/diff_popup.rs` |
...
| **ToolCell** | Inline | `ToolRenderOutput.visual_rows()` — 替换 placeholder 范围 | 打开 `diff_popup`（折叠的命令 / 读取 / 编辑卡片：命中 meta 行末尾的 `双击查看`） |
...
| 双击 card | 打开对应 detail popup；命令 / 读取 / 编辑类卡片已折叠时没有 card，命中目标是 meta 行末尾的 `双击查看`——参数行与该行其余文字都不响应 |

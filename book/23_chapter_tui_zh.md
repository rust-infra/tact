...
| `ToolProgress` | 更新匹配 active tool 的固定五行 live tail |
...
Tool/file detail popup 还支持鼠标左键文本选择。Mouse hit 将每个渲染出的扩展字素簇映射至原始 tool 内容的 byte offset，因此组合字符与 emoji 序列保持不可分割，行号、diff gutter、边框及其他仅用于显示的前缀不会进入选择。Popup 滚动时选择保留；拖拽到 body 上方或下方会 clamp 到首个或末个可见 source boundary，且不会自动滚动。Thinking 与 code detail popup 保持原有鼠标行为。
...
**ToolCell** 取代 placeholder `TextCell`：Phase 3 检测 physical 索引在 `[phys_idx .. phys_idx + placeholder_rows]` 内则在该 block visual start 推一个 cell，跳过剩余 placeholder logical 行。运行中 tool 传 `started_at` 作 live duration，并持有有界 `live_output` buffer。首个可见 `bash` 输出只把 card 展开一次，形成稳定五行；后续 chunk 原位更新 tail。stdout 用普通文本，stderr 用 warning 色；双击 popup 可查看最多 50,000 字符的已缓冲输出。完成后折叠为现有 compact card，并以 `StepResult.detail` 为准。
...
| 在 tool detail popup 内左键拖拽 | 选择原始 tool 文本；排除仅用于显示的前缀 |

复制（normal 模式 `y`）在 tool popup active 时优先非空 popup 选择；popup 选择为空时复制完整原始 popup 内容。无 tool popup 时，优先 log 词选，然后拼接选中 logical 行的 `raw_messages`。

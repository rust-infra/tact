...
`crates/tact-ui/src/headless.rs` 中的 `run_headless`。无 TUI 运行单次 `agent_loop`，最终文本打印到 stdout，发送桌面通知。使用 config 驱动的权限模式 — 与交互模式相同。
...
1. **Phase 0** — `messages.len()` 变化时重建 `visible_indices` / `phys_to_logical_cache`（部分行隐藏，如折叠 thinking 内部）。
...
4. **Phase 3** — 用 `TextCell`、`ToolCell`、thinking/diff/code overlays 构建 `LogColumnRenderer`；仅绘制与 viewport 相交的 cell。
...
| Thinking overlay | `cells/thinking.rs` | 折叠 thinking card（最多 3 行 preview） |
...
| Thinking detail | 双击 thinking card | `popups/thinking_popup.rs` |
...
| `LLMThinking` | Thinking 标题、`│ …` 内容行、隔离 blank | `LOG_THINKING_INDENT` |
...
| **Thinking 内部** | 灰色斜体 `│ …` 于 title 下 | block 关闭后折叠为 ≤3 可见行；见 [§6.14](#614-可见性规则) |
...
| 注册表 | Key | 用于 |
|--------|-----|------|
| `thinking.blocks[]` | `title_idx`、`end_idx` | 折叠 card + thinking popup |
...
1. **Thinking gate** — 产出内容的 update *除* `ThinkingChunk` / `TokenUsage` / `ModelInfo` 外，若 thinking 区域仍开则调用 `flush_and_close_thinking()` 作安全网。优先显式 `ThinkingChunk::Finished`。
2. **Loading gate** — 多数 update 调用 `remove_loading_placeholder()`。仅元数据（`TokenUsage`、`ModelInfo`）跳过移除。Legacy `PlanGenerated` handler 也跳过，但 agent 从不发出 — loading 行路径 inactive。
...
| **`ThinkingChunk::Started`** | Blank + title 行（`LLMThinking`） | 打开 active thinking 区域 |
| **`ThinkingChunk::Delta`** | 每个 `\n`：`│ …` 行（`LLMThinking`）；若漏 `Started` 则打开区域 | 自动 scroll |
| **`ThinkingChunk::Finished`** | Flush 剩余 buffer；折叠为 `ThinkingBlock` | 关闭 active 区域 |
...
| `thinking.buffer` | `ThinkingState` | `ThinkingChunk::Delta` 期间 `\n`，或 `Finished` / safety-close 时 flush |
...
```text
ThinkingChunk::Started →  blank + title 行，设 active_start
ThinkingChunk::Delta   →  追加 │ 内容行，更新 active_end
...
关闭时仅最后三行内容*可见*（`total > 3` 时 `scroll_offset = total − 3`）；更早行仍在 `messages[]` 但被 `is_message_visible` 过滤。紫色 **thinking card overlay**（`render_thinking_cards`）在可见 slice 上绘制折叠 preview。
...
`is_message_visible(phys_idx)`（`widgets/state/app/visibility.rs`）实现 thinking 折叠：

```text
一个 thinking block 的 physical 行：

  [blank]     title_idx          ← 始终可见（title + card chrome）
  [title]     "🧠 Thinking…"
  [+1]        "│ line 1"         ← scroll_offset > 0 时隐藏
  …
  [end_idx]   "│ line N"         ← 仅 [scroll_offset, scroll_offset+3) 中 3 行可见
  [blank]     end_idx + 1        ← 尾随分隔符，可见
```

对每个已关闭 `ThinkingBlock`，`title_idx < idx ≤ end_idx` 的内容行仅当相对索引落在 `[scroll_offset, scroll_offset + 3)` 可见。

**示例** — 8 行 thinking 内容，默认折叠：

| Physical | 内容 | 可见？ | Logical index |
|----------|------|--------|---------------|
| 10 | (blank) | 是 | 5 |
| 11 | title | 是 | 6 |
| 12–16 | lines 1–5 | **否** | — |
| 17–19 | lines 6–8 | 是 | 7–9 |
| 20 | (blank) | 是 | 10 |

Tool placeholder 与 code placeholder 行在 Phase 0 **各自**可见，但 Phase 3 用单个 `ToolCell`（或 code overlay）替换整个 placeholder *范围*。空 `spans` 行与 task-end sentinel 参与 logical 计数但 visual 高度 minimal。
...
**手动 scroll：** 鼠标滚轮与 normal 模式 `j`/`k` 按 logical offset ±1。用户上滚时新流内容不强制 offset，除非 update 显式设 `u16::MAX`（流 chunk 总是设 — 故 live task 会重跟 tail，除非用户每帧继续上滚）。
...
| **Thinking card** | Overlay | 跨越 title 至 trailing blank logical 行 | 打开 `thinking_popup` |
...
**TextCell**（`cells/text.rs`）正常绘制 clone cache wrap 行。选择应用 `REVERSED`（词级或整行）。`total > 3` 时首行前缀折叠 thinking（`↑ 3/8 blocks hidden ↑`）。左 gutter `indent_cols` 来自 `RawMessageType`。

**ToolCell** 取代 placeholder `TextCell`：Phase 3 检测 physical 索引在 `[phys_idx .. phys_idx + placeholder_rows]` 内则在该 block visual start 推一个 cell，跳过剩余 placeholder logical 行。运行中 tool 传 `started_at` 作 live duration；完成 block 读 `tools.blocks`。

**为何 thinking/code 用 overlay？** 折叠 thinking 需跨多 logical 行的 bordered card 而不重写 `messages[]`。Code block 同理：流式 fence 行换 blank placeholder + card 内部预渲染 `styled` cache — 热 `render()` 无 markdown 重解析。
...
复制（normal 模式 `y`）优先词选，否则拼接选中 logical 行的 `raw_messages`。

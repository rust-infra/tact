...
| `ThinkingCell` | `cells/thinking.rs` | Direct live card：前后各有一行空白，1→3 行 tail，完成后为 1 行 summary |
...
| **Thinking card** | Placeholder 行（`LLMThinking`） | 一个 `ThinkingCell`；前后各有一行空白与相邻内容分隔，active tail 从 1 增至 3 行，完成后显示 1 行 summary |

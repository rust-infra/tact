...
| `ThinkingCell` | `cells/thinking.rs` | Direct live card：1→3 行 tail，完成后为 1 行 summary |
...
| **Thinking card** | Placeholder 行（`LLMThinking`） | 一个 `ThinkingCell`；active tail 从 1 增至 3 行，完成后显示 1 行 summary |
...
| **`ThinkingChunk::Started`** | 为一行 `ThinkingCell` 保留 placeholder 行 | 打开 active thinking card |
...
| **ThinkingCell** | Inline | Active 1→3 tail 行；completed 一行 summary | 打开 `thinking_popup` |

...
本章说明 Tact 的**磁盘持久化层**：`.tact/` 下的 JSON 文件存储，以及独立的 SQLite 会话数据库。二者共同保存对话历史、领域状态（cron、队友等）与可观测性数据。
...
```rust
root.file::<T>("cron/scheduled_tasks.json")?     // Store<T>  （遗留 cron — 不再读取）
...
- [Cron 调度](./16_chapter_cron_zh.md) — cron SQLite 表布局

...
| 3 | [持久记忆](./03_chapter_memory_zh.md) | `.claude/memory/` Markdown 记忆、类型、系统提示注入、`save_memory`、`MEMORY.md` 索引 |
...
| 14 | [团队协作](./14_chapter_team_zh.md) | `.claude/team/` 队友名册、JSONL 收件箱、广播、计划审批 / 关机协议 |
| 15 | [Worktree 泳道](./15_chapter_worktree_zh.md) | 隔离 `git worktree`：`create` / `list` / `status` / `run` / `events`、索引与审计日志 |
| 16 | [Cron 调度](./16_chapter_cron_zh.md) | 定时 prompt 注册表、`.claude/cron/` 持久化、`cron_create` / `list` / `delete`、运行时缺口 |
...
| 19 | [持久任务管理器](./19_chapter_persistent_tasks_zh.md) | `TaskManager`、`task_create` / `get` / `list` / `update`、`.claude/tasks/` 依赖 |

...
**3. 摘要调用** — 一次新的非流式 `create_message`（无 tools）；选择输入前先预留输出与 10% 安全余量。摘要**文本**部分沿用经典的 `min(窗口 × 20%, 2,000)` 输出预算；当配置了显式 reasoning effort（或 DeepSeek / Kimi K3——其服务端默认 thinking 开启 + effort high）时，会在文本预算**之上**追加 reasoning 预留（`max_tokens` = 文本 + 预留；预留按 effort 分档为文本预算的 25/50/75/100%）。reasoning-effort 类 provider（OpenAI o 系 / DeepSeek / Kimi K3）把 reasoning token 与文本算在同一个 `max_tokens` 信封内，若不预留，高/max effort 会挤占摘要文本额度、触发截断续写。配置的 Claude 式 thinking budget 会像主循环一样传给摘要请求（`with_thinking`），并**限制在线上 `max_tokens` 之下**（Anthropic 拒绝 `budget_tokens >= max_tokens`，而配置的 budget 可能超过封顶后的摘要输出预算），同时同样从输入预留中扣除，因为 thinking token 也占用上下文窗口空间。如果连固定的摘要指令本身都超过这个输入上限，压缩会提前失败，因为即使删除全部历史，也无法构造合法的摘要请求。瞬时传输错误最多退避重试三次；`MaxTokens` 截断的摘要会**续写**（最多三次）：把已产生的部分摘要作为 assistant 消息、追加一条续写提示（`[compact continue n/3]`）再次调用，与主循环的输出上限恢复一致；续写次数耗尽后，部分摘要会被接受为 best-effort（Codex-style 重建反正会保留最近的真实用户消息）。拒绝/其它异常终止原因和空文本仍会被拒绝，旧 context 不会被替换。摘要要求模型保留：

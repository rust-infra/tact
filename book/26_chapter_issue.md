...
|-------|-------|
| **Type** | feature |
| **Related** | `crates/tact-gui/src/shell.rs` (`submit`, `invoke_skill`, `submit_task`, `accept_suggestion`, `suggestion_section`, `opening_state` / `with_workspace` / `switch_workspace`, the `CompactSession` arm); `crates/tact-gui/src/composer.rs` (`Skill`, `Skill::load`, `skill_rows`, `skill_args`, `skill_task`); `crates/tact-gui/src/session.rs` (`SessionState::skills`); `crates/tui/src/handlers/skills.rs` (the framing this mirrors); `crates/tui/src/render/popups/slash_command.rs` (the popup it matches) |

**Symptom / motivation:** The desktop's `/` list was a set of skill *names* that inserted a mention. The terminal's is a command surface: built-ins ahead of skills under "Commands" / "Skills", each row printing what it is for, and Enter **running** the row — a skill expands into its body. Two readers doing the same thing got two different outcomes, and `/am-checkpoint` only meant anything if the agent chose to resolve the mention itself.

**Decision:** The catalogue is the agent's own — `tact::skill::get_skill_registry`, the same roots, plugin skills and `[agent].skill_dirs` the terminal loads — scanned once per workspace beside the file index, and the list carries each skill's description. The rows are headed "Commands" and "Skills", and a heading is deliberately not a row: no id, nothing to click, no place in the index the keyboard counts. Taking a row runs it — a command through `run_palette_command`, a skill through `invoke_skill` — instead of inserting text. A skill invocation sends the terminal's framing: the body wrapped in `<skill name="…">`, bare `$ARGUMENTS` substituted, `ARGUMENTS: …` appended when the body has no placeholder. The transcript keeps the command line the reader typed, so the send path carries two strings (`submit_task(display, task)`); the queue carries the task, since the row is drawn when it is queued. A `/name args` typed in full is the same invocation, with the arguments after the name. Two smaller alignments came with it: `/compact` is refused while a turn is in flight, in the terminal's own words, and `with_workspace` now roots both indices at the workspace it is given — they were being built from the process's directory, so a shell told it was elsewhere still offered the wrong files and skills.

**Behavior after:** `/` lists `/compact` under "Commands" and the installed skills under "Skills", each with its description; Enter or a click runs either, and only `@` rows insert text. A row reads like the terminal's: left-aligned marker, name, and the description taking the rest of the line, cut with an ellipsis when it does not fit; **Tab** completes the name without running it (the terminal's split between Tab and Enter), and only lets go of the key when the list has nothing to give. The row is drawn rather than borrowed from `Button`, which centres its content — that centring is what put the name in the middle of the line until this change. `/am-checkpoint fix auth` submits the skill's body with the arguments applied, and the transcript shows `/am-checkpoint fix auth`. A skill the project root carries is offered alongside the global ones, and a shell rooted at a directory offers that directory's files and skills. `/compact` mid-turn leaves the context alone and says so.

**One implementation, not two:** the command line parsing and the framing (`tact::skill::slash_args` / `tact::skill::slash_task`) are shared, because this shape is the agent's protocol rather than either client's copy — the system prompt keys on `<skill name="…">` and a trailing `ARGUMENTS:` line. The desktop kept a byte-for-byte copy for one revision while the terminal's logic was frozen; that copy is now gone (net −135 lines), the terminal's `skill_args_from_input` / `format_skill_agent_task` delegate to the shared pair, and the framing cases the terminal used to own are tested where the code now lives.

**Pointers:** `crates/tact/src/skill/mod.rs` (`slash_args`, `slash_task`, `get_skill_registry`); `crates/tact-gui/src/composer.rs` (`Skill::load`, `skill_rows`); `crates/tact-gui/src/shell.rs` (`invoke_skill`, `submit_task`, `accept_suggestion`, `suggestion_section`); unit tests `a_skill_row_sends_the_body_and_keeps_the_command_line`, `a_typed_skill_command_carries_its_arguments`, `compact_is_refused_while_a_turn_is_in_flight`, `skills_load_through_the_agent_loader_with_their_bodies`, and in `tact::skill` `slash_arguments_come_from_after_the_name`, `slash_task_wraps_the_body`, `slash_task_substitutes_the_bare_arguments_placeholder`, `slash_task_leaves_indexed_and_longer_arguments_tokens`; `crates/tact-gui/tests/shell.rs` (`the_slash_list_groups_commands_and_skills`); `crates/tui/src/handlers/skills.rs` (the terminal's delegating call sites)
...
**Behavior after:** `/compact` in the composer compacts the session; `[compacting]` and `Compaction complete.` arrive as system rows and the command itself never becomes a message. Typing `/` lists `/compact` first with "Compact conversation history" beside it, ahead of the skills; Enter (or a click) on that row runs the command and empties the draft. File rows keep their older meaning — a mention is inserted, not run. (Skill rows were still inserted at this point; the entry above replaces that with the terminal's own invocation.) Adding an entry to `SLASH_COMMANDS` is what makes another palette command reachable from the composer, in the list and on submit alike.

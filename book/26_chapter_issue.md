...
**Behavior after:** Assistant output, reasoning, and tool activity now run through gpui-ai's streaming and disclosure lifecycles. Consecutive tool calls share one gpui-ai `ToolGroup`, and a pending permission approval uses gpui-ai `ApprovalCard` nested directly beneath that tool run instead of floating as a separate trailing card. Single-choice `ask_user` requests use gpui-ai `QuestionFlow`; multi-select Ask keeps its special Confirm/Cancel form. Thinking and tool cards each expose a copy control for the full underlying text. Tool output format is selectable (`Markdown` or `Plain`); Tact selects `Plain` for command output, where line breaks and indentation survive without Markdown code-block padding. Headers stay fixed while output bodies grow with the content up to a 190px maximum, then scroll with an always-visible scrollbar; write rows keep their clickable diff badge. The model picker uses compact, left-aligned rows inside an always-visible scrolling viewport. Provider/subagent loading states use the gpui-ai pixel-grid loader. The branch no longer carries Tact's custom Markdown block renderer or Mermaid fence renderer.

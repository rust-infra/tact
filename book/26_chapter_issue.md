...
**Note on the duplicated policy:** the invocation framing now exists twice — the terminal's `format_skill_agent_task` and the desktop's `composer::skill_task` — because the terminal's logic is frozen. Ported tests pin the same cases on both sides, but the honest fix is one shared helper in `tact::skill`; that is a terminal-side change, so it stays its own decision.

**Pointers:** `crates/tact-gui/src/composer.rs` (`Skill::load`, `skill_rows`, `skill_args`, `skill_task`); `crates/tact-gui/src/shell.rs` (`invoke_skill`, `submit_task`, `accept_suggestion`, `suggestion_section`); unit tests `a_skill_row_sends_the_body_and_keeps_the_command_line`, `a_typed_skill_command_carries_its_arguments`, `compact_is_refused_while_a_turn_is_in_flight`, `skills_load_through_the_agent_loader_with_their_bodies`, `a_skill_invocation_is_framed_the_way_the_terminal_frames_it`; `crates/tact-gui/tests/shell.rs` (`the_slash_list_groups_commands_and_skills`); `crates/tact/src/skill/mod.rs` (`get_skill_registry`); `crates/tui/src/handlers/skills.rs`

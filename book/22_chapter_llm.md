...
**User image attachments (de-inlined):** TUI/headless keeps `@file.png` / `![alt](path)` as **path text** in the user message rather than base64-inlining it. The model reads the reference on demand with the `read_image` tool (image) or `read_file` tool (text); see the `read_image` paragraph below. Previously the attach step base64-decoded the image into an inline `ContentBlock::Image`; that path was removed so image bytes only enter the request when the model asks for them. A text-only Chat Completions API that still receives an inline `image_url` part rejects it with HTTP 400.

**`read_image` tool (Harness-style):** the agent toolset includes `read_image` (a model-callable tool over `file_path`). It gates on the current model declaring image input (`tact_llm::supports_vision`), reads a PNG/JPEG/WebP/GIF, re-encodes to a bounded JPEG, and returns a `ToolCallResult` carrying both a text envelope (`<path>…</path>`, media type, dimensions, bytes) and an adjacent `ContentBlock::Image`. Because Chat Completions `role:tool` content is string-only, `messages_to_openai` folds a tool-result image into a following `role:user` message (`[{type:"text", text:"Attached image(s) from tool result:"}, {type:"image_url",…}]`), mirroring DeepSeek Harness `serializeMessagesWithImages`. This keeps text-only tool results on `role:tool` and lets a vision model consume the image from the user message without changing `ContentBlock::ToolResult`.

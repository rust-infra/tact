...
子 agent **不**获得 cron、团队、任务管理、仅 MCP 名称或其他特权工具。模块注释写四个工具，实现里是六个——以上方 `route()` 列表为准。完整 spawn 生命周期：[Subagents](./12_chapter_subagent.md)（英文）。
...
| Router 注释漂移 | `subagent_toolset` 文档写 4 个工具；代码注册 6 个 |

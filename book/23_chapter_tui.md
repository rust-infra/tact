...
**Top bar** (`render_status_bar`): input mode, `Status` (Idle / Planning / Executing / WaitingForUser / Done), theme/language hints. Overrides: temporary `flash_msg`. No panel-focus label (single-column log only). While `Executing` it also carries the step label (`Executing step 4/10` / `正在执行步骤 4/10`) and the parallel-tool count (`running 1` / `并行中 1`); the `[████░░] n%` gauge and the live task clock were both removed on 2026-09-14 — the gauge only restated the step count as glyphs, and the clock moved down to bottom-bar row 2, next to the frozen turn timing (see below), so the status bar renders no numbers of its own.

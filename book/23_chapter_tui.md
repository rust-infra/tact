...
**Top bar** (`render_status_bar`): input mode, the focused-panel label (`FocusedPanel` is `Log`-only today, but every status arm renders the slot), `Status` (Idle / Planning / Executing / Done), theme/language hints in `Idle`. Overrides: temporary `flash_msg`. While `Executing` it also carries the step label (`Executing step 4` / `正在执行步骤 4`, no denominator since 2026-09-14) and the parallel-tool count (`running 1` / `并行中 1`); the `[████░░] n%` gauge and the live task clock were both removed on 2026-09-14 — the gauge only restated the step count as glyphs, and the clock moved down to bottom-bar row 1, next to the uptime (see below), so the status bar renders no numbers of its own.

**Bottom bar** (`render_bottom_bar`, always 2 rows):
- Row 1: permission mode, cwd, uptime (`⊙ Up …` / `⊙ 运行 …`), the live task elapsed (`⏱ Elapsed 00:12` / `⏱ 耗时 00:12` — directly after the uptime, because the two clocks that describe *this run* belong together; omitted when no task is in flight, and pushed as the row's last droppable so it is the first row-1 segment dropped on a narrow terminal), git branch (`⎇`), optional account (`¤ …` for DeepSeek / Kimi). Segments joined with ` │ `. Prompt elapsed lives on the **task-end separator** (not the bottom bar).
- Row 2: model name, `out` (`max_tokens` **verbatim** — the number the request is actually sent with: `max_output_tokens` on the Responses protocol, `max_tokens` on chat completions / Anthropic. Reasoning is **not** subtracted even for effort-semantic models, whose reasoning is counted inside this same envelope: the reasoning/text split is the endpoint's per-request decision, so the bar reports what was asked for rather than guessing — estimating a reasoning reserve is the compaction path's job, not a readout's. See the 2026-09-13 entries), `think high`/`思考 high` (effort) or `think 32K`/`思考 32K` (budget; the two are mutually exclusive — a stale budget is never shown next to an effort), `ctx` usage (`ctx 4% 45K/1M` — percentage first, then the absolute used/window; the gauge was dropped on 2026-09-12 because it only restated the percentage as glyphs), `▣` cache hit rate, turn counters (`⟳ 12` for session user turns, plus `⇅ 3` for the current task's agent-loop turns — hidden until the first LLM call of the task), and turn timing (`⏱ 02:05` for the last completed turn plus `avg 01:45`/`均 01:45` across the session; the average is omitted until a turn completes). Segments joined with two spaces. Narrow terminals drop timing → turns → cache → ctx first, i.e. `ctx` survives longest.

**Row-2 compaction (2026-09-12):** adding the turn segments pushed row 2 to ~138 columns, so segments began dropping on ordinary terminals. The row was cut to **90 columns** with no loss of distinct information (86 after the same-day ctx revision described next). The governing rule is *one value, one rendering*: (1) the `∑ₜₒₖ {total}` segment was **deleted** — it read the same `StatusBarState.token_total` the `ctx` meter renders as its `used` figure (the exact integer survives in the task-stats block and `/stats`); (2) `max_out_token` → `out`; (3) `cache%` → bare `▣ 30%`; (4) the `turns` word was dropped from both counters, leaving `⟳ 12 ⇅ 3`. The width budget is pinned by `bottom_bar_fits_every_segment_in_100_columns` (85–86 columns, depending on the `out` value); the task clock that row 1 gained on 2026-09-14 has its own budget test (`bottom_bar_fits_the_task_elapsed_on_row_1_in_100_columns`).

**ctx percentage restored, gauge dropped (2026-09-12, same day):** the compaction above briefly removed the ctx `pct%` and kept the `■`/`·` gauge. Reversed the same day: the gauge is **gone** and the percentage leads — `ctx [▍···] 45K/1M` → **`ctx 4% 45K/1M`** (24 → 17 → 14 columns). Rationale: the gauge only restated the percentage as glyphs, whereas `45K/1M` alone forced the reader to do the division to answer "how close am I to auto-compact". The absolute `used/window` stays — a ratio cannot replace the two counts it came from. The cache `▣` segment also moved to sit directly **after `ctx`** and before the turn counters, so both session-wide ratios read together; push order is now `model → out → think → ctx → cache → turns → timing` and narrow-terminal survival is `ctx > cache > turns > timing`.

**Turn counters and timing (2026-09-12):** `⟳` counts user turns dispatched this session — incremented at the single dispatch choke point (`handlers/skills.rs::dispatch_user_task`, which also serves queued flushes and skill dispatch) and seeded on resume by counting persisted user messages in `load_history`. `⇅` counts agent-loop iterations for the current task: the agent emits `AgentUpdate::TurnStats { turns_taken, max_turns }` once per loop iteration (`crates/tact/src/agent/mod.rs`), the kit's `StatusBarComponent` stores it, and the shell resets it at dispatch. `max_turns` is plumbed into `StatusBarState.turn_llm_cap` but deliberately **not rendered** — only `spawn_subagent` ever sets a cap, so a main-agent bottom bar would never show one. Timing accumulates in `add_task_end_separator` (`widgets/state/app/popups.rs`), the only place that actually freezes `task_start_time`; cancelled turns count, synthetic separators (no start time) do not. The live in-flight elapsed is the separate **row-1** segment described above (`format_task_elapsed` from `ctx.task_start_time`, next to the uptime): since 2026-09-14 it lives there and the top status bar shows no clock at all. It deliberately sits on the other row from this frozen timing — row 1 answers "how long has this run taken", row 2 "how long did the finished turns take" — and being the last droppable of its row it is also the first segment to go when columns run short.

**Input** (`render_input_box`): rounded border in `Insert` mode; up to 3 content rows; long lines soft-wrap at character boundaries (`wrap_line`, CJK double-width aware — `Paragraph` stays unwrapped and draws exactly those rows) and the caret/scroll follow the wrapped rows (`caret_in_wrapped`); CJK-aware cursor width; no approval banner (an agent permission prompt is a `RequestSelect` update that opens the select popup in `InputMode::Select`). Palette mode uses `render_command_line`. When `[voice].enabled = true`, a **centered** title-bar button (separate `Block` title from the left input label, so the top border stays visible between them) records microphone audio (macOS permission required), sends WAV to the configured transcription service, and inserts the returned text at the cursor (`Esc` cancels). Optional `[voice].voice_keybind` toggles the same control from the keyboard; only an exact match is consumed. See [Ch 21](./21_chapter_config.md) and `crates/tact/src/voice/`.
...
- Streaming buffers the whole block until its closing fence; when rendering succeeds, the diagram lines are spliced into the log directly and **no code card** is created. The fence body is retained in a `MermaidBlock` so double-click can open the popup.
- **Double-click** any diagram row → Mermaid popup. Since 2026-09-11 the popup opens on the **rendered diagram**, re-laid out at the popup's width (~80% of the frame) rather than the narrower log panel, so dense flowcharts stay readable; **`Tab`** switches to the raw fence body and back. **`y`** copies the Mermaid source in either view. Log selection / Normal **`y`** still copies the visible ASCII diagram.
- A diagram that cannot be rendered (e.g. Mermaid `style` / `classDef` / `linkStyle`, which the pinned renderer does not parse) shows its source in the popup with an explicit "does not render" note instead of silent blank art.
- Invalid or unsupported Mermaid falls back to the normal code block/card in the log, so the original source stays readable.

...
**改后行为：** 助手输出、reasoning 和工具活动都由 gpui-ai 的 streaming / disclosure 生命周期驱动。连续工具调用合并为一个 gpui-ai `ToolGroup`，等待中的请求直接嵌套在发起它的工具块下：permission 使用 gpui-ai `ApprovalCard`，单选和多选 Ask 选择项都留在 Ask 工具卡内部。单选的 `ask_user` 使用 gpui-ai `QuestionFlow`；多选 Ask 保留特殊的 Confirm/Cancel 表单，但去掉嵌套卡片的上边框，使它读起来像工具卡正文的一部分。空的工具 output 区域会被省略。pending Ask / permission 会让所属 session 继续驻留，切换出去再回来时卡片不会丢失。Thinking 和工具卡片各自提供 full-content copy 按钮。运行中的工具输出采用和 Thinking 一样的 live-tail 行为：只要用户没有滚上去查看早期内容，新输出会把 output scroller 固定在底部；用户开始查看早期行后不再抢位置。工具输出格式可选（`Markdown` 或 `Plain`）；Tact 对命令输出选择 `Plain`，换行和缩进都会保留，同时避免 Markdown code block 的额外缩进。工具 header 保持固定，output body 随内容增长，最多到 190px 后滚动，并带始终可见的滚动条；同一套 nested-scroll capture 会在展开的 Thinking / 工具卡片还有滚动空间时把滚轮事件留在卡片内，到达边缘后再交还给转录列表。write row 仍显示可点击 diff badge。Files preview 可以把当前文件打开到内嵌 Neovim（使用 `--listen` socket）；`Nvim selection` popover 会先确认，再把 Neovim 最近一次 visual selection 以引用文本块插入 composer。Session menu、composer add menu、model picker 和 settings font picker 统一使用紧凑、左对齐的行语法；长列表在自身区域内滚动。Transcript 的 virtual count 现在和 renderer 使用同一套 nested-request 判断，permission 挂在工具块下时不会再预留一行空白。Terminal resize 按 vt100 的原生 `(rows, cols)` 顺序比较。Provider 与 subagent 加载态使用 gpui-ai 像素网格 loader。该分支不再保留 Tact 自带的 Markdown block renderer 或 Mermaid fence renderer。

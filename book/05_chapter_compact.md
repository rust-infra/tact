...
|---------|---------|-------|
| `agent.model_context_window` | **200,000** | Tokens; CLI `--model-context-window` / TOML. Breaking rename from `context_limit_chars` — **no silent alias**. Resolution: CLI > `[agent]` > built-in model→window mapping (e.g. `deepseek-v4-pro`, `claude-opus-4-7`, `gpt-5.6-sol` → 1M) > default 200,000. The mapping is a fallback for unconfigured models, so a stale manual value can under-report a long-context model. |
...
**1. Transcript spill** — `write_transcript` atomically creates a unique `.tact/transcripts/transcript_<unix_nanos>_<collision>.jsonl`, one JSON message per line. TUI shows `[transcript saved: …]`. Full history is recoverable offline, and the handoff cell the model receives ends with that path (`Full pre-compaction transcript: … — read it selectively if you need detail this summary dropped.`), so a continuing agent can tell that early turns are recoverable instead of assuming the summary is all that is left.
...
**3. Summarization call** — a fresh non-streaming `create_message` (no tools) reserves output and 10% safety headroom before selecting input. The summary **text** keeps the classic `min(20% × window, 2,000)` output budget. The summarizer never forwards a Claude-style thinking budget (thinking adds little value to a handoff summary), and its reasoning reserve is the absolute token **bucket** of the attempt's effective effort — `none` 0, `minimal`/`low` 2,000, `medium` 4,000, `high` 8,000, `xhigh`/`max` 16,000 — added **on top** of the text budget (`max_tokens` = text + bucket). Where the request can spend that envelope on reasoning — any effort-semantic provider (OpenAI / DeepSeek / Kimi k3 / custom with a configured effort, including the server-default tiers) — the wire `max_tokens` is additionally floored by `[agent] max_tokens`, capped so the fixed instructions still fit the window. Such a provider has **no separate thinking budget**, so an envelope below the configured output budget can be consumed by thinking alone (measured: `max_tokens = 4000` → `reasoning_tokens = 4000`, zero summary text, while DeepSeek's own thinking-mode default is 64K). Budget-semantic providers (Anthropic) never receive a thinking budget here and keep the classic text cap. The reserve runs a **staged effort ladder**:
...
Reasoning counts inside the same `max_tokens` envelope as the text, so without the reserve a reasoning model would starve the summary — and the reserve is bookkeeping only: the wire carries one envelope, so an under-sized stage 0 buys nothing at all. Measured on a DeepSeek-backed gateway (`deepseek-flash`, `reasoning_effort = low`): stage 0 sent `max_tokens = 4000 (text 2000 + reasoning 2000)` and came back `MaxTokens` with `reasoning_tokens = 4000` and zero summary text; the next stage (`max_tokens = 2000`, reserve 0) then produced the summary in 649 reasoning + ~465 text tokens, because it *continues* from the partial reasoning instead of re-deriving it. If the fixed summary instructions alone exceed the input limit, compaction fails early because no valid summary request can be constructed, even after removing all history. Transient transport failures get up to five retries with backoff. Every summary attempt logs its own envelope — `[compact summary n/6] request … max_tokens=… (text … + reasoning …), reasoning_effort=…, input … chars` before the call and `[compact summary n/6] response stop=…, prompt …, completion … (reasoning …), cache …/…` after it, emitted on the first-try success path too — so both the `max_tokens` actually sent and what the call really cost are always in the transcript. Stop reasons print in the same snake_case vocabulary the rest of the agent's messages use (`stop=end_turn`, `stop=max_tokens`, `stop=unknown(<raw>)`), and no usage is reported as `no usage reported` rather than an empty dump. A `MaxTokens` truncation is **continued** (up to `MAX_COMPACT_SUMMARY_ATTEMPTS` = 5, `[compact continue n/5]`): the partial summary is appended as an assistant message plus a continuation prompt, and the notice names **both** units of what the attempt spent — `summary truncated (4000 reasoning tokens, thinking block 15314 bytes), next attempt max_tokens=2000` — the billed reasoning tokens first, the byte weight of the thinking block second, so a byte count is never read as a token count. When the ladder is exhausted the partial summary is accepted as best-effort with `[compact fallback]` (the Codex-style rebuild keeps recent real user messages anyway). Refusal / other abnormal stop reasons and empty text are still rejected without replacing the old context. The instructions also state that the conversation appears as a JSON message array (tool results and attachments may be elided) and that the summary must be grounded in it. The prompt asks the model to preserve:
...
|---------|---------|--------|
| `agent.model_context_window` (`--model-context-window`) | 200,000 | Token window: auto-compact at 80% + TUI usage meter; when nonzero it must exceed `max_tokens`. Resolution: CLI > `[agent]` > model→window mapping (e.g. `deepseek-v4-pro` → 1M) > default |
| `agent.max_tokens` | 8,000 (Kimi K2.x 32,000) | Reply budget, the post-compaction rebuild reservation, the auto-trigger reservation — and the summarizer's envelope **floor** on effort-semantic providers (capped so the instructions still fit the window); budget-semantic providers keep the classic `min(20% × window, 2,000)` text cap |
...
| Only recent 20k estimated tokens summarized | Early turns live in the transcript only; the handoff cell names the file (which turns are summarized away is still decided by tail selection, not by importance) |

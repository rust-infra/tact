...
| `--unshare-net` in the §10 flag list | `--share-net`, plus `--ro-bind /run/systemd/resolve` and the proxy variables forwarded verbatim | Changed 2026-09-16 on the owner's request: the sandbox bounds the filesystem, not connectivity. Measured: sharing the namespace alone leaves `/etc/resolv.conf` (a symlink into `/run/systemd/resolve`) dangling, so lookups fail; and on a host whose only route out is a proxy, dropping the proxy variables turns a working command into a connection error. Binding `/etc/resolv.conf` directly is refused by bwrap (`Can't mount on symlink destination`). |
| `fn bwrap_args_with(work_dir, exists)` | `fn bwrap_args_with(work_dir, exists, env)` | The environment allowlist is a rule like the mount list, so it is driven by an injected lookup and unit-tested without touching the process environment. |

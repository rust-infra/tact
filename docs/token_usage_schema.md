...
The data lives in the `token_usages` table inside the session database at `<workdir>/.tact/tact.db` (one DB per project working directory). Duplicating a session deliberately copies only its messages, not its `token_usages` or provider state: recorded spend stays attributed to the original, and the copy starts a fresh provider chain.

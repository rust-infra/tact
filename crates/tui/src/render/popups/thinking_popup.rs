...
fn is_ordered_list_item(line: &Line<'_>) -> bool {
    ...
    digits > 0
        && trimmed.as_bytes().get(digits).is_some_and(|byte| *byte == b'.')
        && trimmed.as_bytes().get(digits + 1).is_some_and(u8::is_ascii_whitespace)
}
...
pub(crate) fn render_thinking_popup(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let (styled_lines, raw_total) =
        if let Some(active) = app.thinking.active.as_ref().filter(|active| active.phys_idx == popup.phys_idx) {
            let lines = active.content.lines().map(|line| Line::from(line.to_string())).collect::<Vec<_>>();
            (lines, active.content.lines().count())
        } else if let Some(block) = app.thinking.blocks.iter().find(|block| block.phys_idx == popup.phys_idx) {
    ...
    let selection_text = styled_lines.iter().map(line_text).collect::<Vec<_>>().join("\n");
    let selection = popup.selection.and_then(|selection| selection.normalized_non_empty(&selection_text));
    ...
    for (index, source_line) in source.iter().enumerate() {
        let styles = scalar_styles(styled_lines.get(index), fallback, source_line.text.chars().count());
        display_rows.extend(
            layout_display_rows(source_line.text, source_line.start, &styles, body_area.width as usize, true)
                .into_iter()
                .map(ThinkingDisplayRow::Content),
        );
        if styled_lines.get(index).is_some_and(is_ordered_list_item)
            && styled_lines.get(index + 1).is_some_and(is_ordered_list_item)
        {
    ...
    frame.render_widget(
        Paragraph::new(Line::from(Span::styled(
            format!("{} ({} markdown lines, {} raw)", popup.title, styled_lines.len(), raw_total),
            title_style,
        ))),
        header_area,
    );
    ...
    for (visible_row, display) in display_rows.iter().skip(scroll).take(content_height).enumerate() {

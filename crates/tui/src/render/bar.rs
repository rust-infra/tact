...
fn format_tokens_compact(n: u64) -> String {
    if n < 1_000 {
        ...
        if (k * 10.0).round() % 10.0 == 0.0 { format!("{:.0}K", k) } else { format!("{:.1}K", k) }
    } else {
        let m = n as f64 / 1_000_000.0;
        if (m * 10.0).round() % 10.0 == 0.0 { format!("{:.0}M", m) } else { format!("{:.1}M", m) }
    }
}
...
fn context_usage_pct(used: u32, window: usize) -> u8 {
    if window == 0 { 0 } else { ((used as u128) * 100 / window as u128).min(100) as u8 }
}
...
fn format_context_meter(used: u32, window: usize) -> String {
    ...
    format!("{bar} {pct}% │ {}/{}", format_tokens_compact(used as u64), format_tokens_compact(window as u64))
}
...
fn format_account_suffix(app: &App) -> Option<String> {
    ...
    if let Some(bi) = &app.account.balance {
        let status = if bi.is_available { msgs.bottom_balance_ok } else { msgs.bottom_balance_err };
        let entries: String = bi
            ...
            .collect::<Vec<_>>()
            .join(" |");
        return Some(msgs.bottom_balance_tmpl.replacen("{}", status, 1).replacen("{}", &entries, 1));
    }
    if let Some(quota) = &app.account.quota {
        let status = if quota.is_available { msgs.bottom_balance_ok } else { msgs.bottom_balance_err };
        let entries: String = quota
            ...
            .map(|w| {
                ...
                if let Some(pct) = w.usage_pct() {
                    format!(" {}:{:.0}% {} {}/{}", w.label, pct, render_usage_bar(pct), remaining, limit)
                } else {
            ...
            .collect::<Vec<_>>()
            .join(" |");
        return Some(msgs.bottom_usage_tmpl.replacen("{}", status, 1).replacen("{}", &entries, 1));
    }
    None
}
...
pub(crate) fn render_bottom_bar(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let branch =
        if app.status_bar.git_branch.is_empty() { msgs.bottom_branch_unknown } else { &app.status_bar.git_branch };
    ...
    let elapsed = if let Some(start) = app.task_start_time {
        let secs = chrono::Local::now().signed_duration_since(start).num_seconds().max(0);
        format_mm_ss(secs)
...

    {
        ...
        let style = Style::default().bg(app.theme.bottom_bar_bg).fg(app.theme.bottom_bar_fg);
...
pub(crate) fn render_status_bar(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let (status_text, status_style) = match &app.status {
        ...
        Status::Executing { current_step: _, total } => {
            ...
            let step_label = msgs.status_executing_tmpl.replacen("{}", &display_step.to_string(), 1).replacen(
                "{}",
                &total.to_string(),
                1,
            );
            let running_label = msgs.status_running_tmpl.replacen("{}", &running.to_string(), 1);
            ...
            let progress_idx = if *total == 0 { 0 } else { completed.min(total.saturating_sub(1)) };
            ...
            (
                ...
                Style::default().bg(app.theme.status_bar_bg).fg(app.theme.warning),
            )
        },
        Status::Done => (
            format!("{} {} │ ✅ {}", mode_str, focus_str, msgs.status_done_tmpl.replace("{}", "")),
            Style::default().bg(app.theme.success).fg(app.theme.fg).add_modifier(Modifier::BOLD),
        ),
    };
    let (display_text, display_style) = if let Some((ref msg, _)) = app.flash_msg {
        (format!("⚠ {}", msg), Style::default().bg(app.theme.warning).fg(Color::Black).add_modifier(Modifier::BOLD))
    } else {
...
mod render_tests {
    ...
    fn bottom_bar_renders_without_panic_when_idle() {
        ...
        terminal.draw(|frame| render_bottom_bar(frame, Rect::new(0, 0, 100, 2), &app)).expect("draw");
    ...
    fn bottom_bar_shows_context_usage_meter_on_row_2() {
        ...
        let mut terminal = Terminal::new(backend).expect("terminal");
        terminal.draw(|frame| render_bottom_bar(frame, Rect::new(0, 0, 160, 2), &app)).expect("draw");
        ...
        assert!(row2.contains('[') && row2.contains(']'), "row 2 should include progress bar brackets, got:\n{row2}");
    }
    ...
    fn bottom_bar_shows_elapsed_and_uptime_on_row_1() {
        ...
        let mut terminal = Terminal::new(backend).expect("terminal");
        terminal.draw(|frame| render_bottom_bar(frame, Rect::new(0, 0, 140, 2), &app)).expect("draw");

        let text = buffer_text(terminal.backend().buffer());
        let lines: Vec<&str> = text.lines().collect();
        assert!(lines.len() >= 2, "bottom bar should render two rows, got:\n{text}");
        let row1 = lines[0];
        let row2 = lines[1];
        assert!(
        ...
        assert!(row2.contains("Tok:42"), "token stats should stay on row 2, got:\n{row2}");
    }
    ...
    fn bottom_bar_shows_reasoning_effort_with_thinking_budget() {
        ...
        terminal.draw(|frame| render_bottom_bar(frame, Rect::new(0, 0, 120, 2), &app)).expect("draw");

        let text = buffer_text(terminal.backend().buffer());
        assert!(text.contains("think=32000 (high)"), "bottom bar should show budget and effort, got:\n{text}");
    }
    ...
    fn bottom_bar_preserves_thinking_budget_when_effort_is_unavailable() {
        ...
        let text = buffer_text(terminal.backend().buffer());
        assert!(text.contains("think=32000"), "bottom bar should preserve the budget display, got:\n{text}");
        assert!(!text.contains("think=32000 ("), "bottom bar should omit a missing effort, got:\n{text}");
    }
}

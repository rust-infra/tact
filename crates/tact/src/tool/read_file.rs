...
pub async fn read_file(ctx: ToolContext, input: ReadFileInput) -> Result<String> {
    ...
    let offset = input.offset.map(|o| o.saturating_sub(1) as usize).unwrap_or(0);
    if offset > 0 {
        if offset >= lines.len() {
...
mod tests {
    ...
    async fn read_file_errors_when_file_missing() {
        ...
        let error = run_tool(&context, ReadFileTool, "read_file", serde_json::json!({ "path": "missing.txt" }))
            .await
            .unwrap_err();
    ...
    async fn read_file_returns_empty_when_offset_past_end() {
        ...
        let output =
            run_tool(&context, ReadFileTool, "read_file", serde_json::json!({ "path": "short.txt", "offset": 99 }))
                .await
                .unwrap();

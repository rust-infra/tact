...
//! - [`CollectionStore<T>`] is a directory of keyed JSON files (one file
//!   per key) supporting `list`, `read`, `write`, `append`, and `delete`.
...
/// Root directory for all on-disk stores.
///
/// All paths must be relative; absolute paths and directory traversal are
/// rejected.  The root is created on construction if it doesn't exist.
#[derive(Clone, Debug)]
...
///
/// Supports `read`, `write`, `update` (read-modify-write), `append`
/// (JSONL), `read_all` (all lines), `delete`, and `exists`.
#[derive(Clone, Debug)]
pub struct Store<T> {
    path: PathBuf,
...
/// A directory-based collection of keyed JSON files.
///
/// Each key maps to a `{key}.json` file.  Supports `list`, `read`, `write`,
/// `append`, `delete`, `exists`, and `read_all_from`.
#[derive(Clone, Debug)]
    ...
    T: Serialize + DeserializeOwned,
{
    ...
    pub fn list(&self) -> Result<Vec<T>> {
            ...
            .with_context(|| format!("failed to read {}", self.dir.display()))?
        {
            ...
            // Skip index files that live alongside collection items

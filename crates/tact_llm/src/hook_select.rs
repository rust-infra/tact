...
pub fn body_hook_for(info: &ProviderInfo, user_id: Option<&str>) -> Result<Arc<dyn OpenAiBodyHook>, LlmError> {
    ...
    match info.provider {
        ...
        ProviderKind::Anthropic => {
            Err(LlmError::Other("cannot use anthropic provider with openai-compatible body hooks".to_owned()))
        },
    }
}
...
mod tests {
    ...
    fn body_hook_for_selects_by_kind_and_heuristics() {
        ...
        let openai_kimi_url = provider(ProviderKind::OpenAi, "kimi-k2.5", "https://api.moonshot.cn/v1");

        let request = sample_request_with_thinking();
        let mut deepseek_body = empty_body();
        body_hook_for(&deepseek, Some("u1")).unwrap().inject(&mut deepseek_body, &ctx(&request, &deepseek, &[]));
        ...
        body_hook_for(&kimi, None).unwrap().inject(&mut kimi_body, &ctx(&request, &kimi, &[]));
        assert_eq!(kimi_body["thinking"]["type"], "enabled");
        assert!(kimi_body.get("reasoning_effort").is_none());

        let mut heur_body = empty_body();
        body_hook_for(&openai_kimi_url, None).unwrap().inject(&mut heur_body, &ctx(&request, &openai_kimi_url, &[]));

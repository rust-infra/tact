...
pub fn init_provider_with_credentials(
    ...
    let mut guard = PROVIDER.write_recover();
    *guard = Some(info);
    drop(guard);
    let mut guard = CREDENTIALS.write_recover();
...
pub(crate) fn try_active_credential_provider() -> Option<Arc<dyn CredentialProvider>> {
    CREDENTIALS.read_recover().clone()
}
...
{
    let guard = PROVIDER.read_recover();
    f(guard
        .as_ref()
        .expect("LLM provider not initialized; call tact_llm::init_provider first"))
}
...
/// The [`ProviderKind`] the active provider should be *routed* as, or `None`
/// when no provider has been installed yet.
///
/// Unlike [`read_provider`] this never panics, so library code that may run
/// before `init_provider` (subagent construction, tests) can use it as a
/// default. A generic OpenAI-compatible endpoint pointed at DeepSeek is
/// reported as [`ProviderKind::DeepSeek`], matching [`is_deepseek`].
pub fn current_provider_kind() -> Option<ProviderKind> {
    PROVIDER.read_recover().as_ref().map(|info| {
        if info.provider == ProviderKind::DeepSeek
            || info.base_url.contains("deepseek")
            || info.model.contains("deepseek")
        {
            ProviderKind::DeepSeek
        } else {
            info.provider.clone()
...
pub fn model_uses_effort(model: &str, info: &ProviderInfo) -> bool {
    // Single source of truth: the profile's heuristics, so the `/model` picker
    // and the wire body hooks can never drift apart.
    info.to_profile().model_uses_effort(model)
}

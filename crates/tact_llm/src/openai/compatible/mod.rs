...
//!
//! Serves OpenAI, DeepSeek-compatible, Kimi, and custom OpenAI-compatible
//! endpoints over the Chat Completions API. Uses `async-openai` types for
//! request construction but handles responses via custom deserialization
//! structs in order to capture the `reasoning_content` field, which
//! `async-openai` does not expose in its Chat Completions types
//! (as of 0.40.2).
//!
...
pub struct CreateChatCompletionRequest {
    ...
    ///
    /// `u32` (not `u16`) because models such as Claude Opus 4 and GPT-5
    /// support outputs well beyond 65,535 tokens.
    #[serde(skip_serializing_if = "Option::is_none")]

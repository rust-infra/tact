//! Live Kimi / Moonshot tests for `reasoning_content` echo rules.
//!
//! Official docs (Preserved Thinking / tool turns): historical tact.db data
//! shows Kimi almost always echoed `reasoning_content`; docs say tool turns
//! require it. These tests pin the live API contract.
//!
//! Thinking wire shape: `thinking: { type: "enabled" }` without
//! `reasoning_effort`. For `kimi-for-coding` / K2.7, omit the `thinking`
//! param entirely (see [`crate::kimi`] heuristics) — set `KIMI_MODEL` to
//! exercise that path.
//!
//! Scenarios covered:
//! 1. Plain multi-turn **without** echoing `reasoning_content`.
//! 2. Plain multi-turn **with** echoing `reasoning_content`.
//! 3. Tool-call turn **without** echoing `reasoning_content` (docs: may 400).
//! 4. Tool-call turn **with** echoing `reasoning_content` (docs: required).
//! 5. Tool history with **only the latest** `reasoning_content` kept.
//!
//! Skips when `KIMI_API_KEY` is unset or empty.
//! Also soft-skips on HTTP 402 (Coding membership inactive).
//! Optional: `KIMI_BASE_URL` (default `https://api.moonshot.cn/v1`),
...
fn skip_unless_api_key() -> Option<(String, String, String)> {
    ...
    let api_key = match std::env::var("KIMI_API_KEY") {
        ...
        _ => {
            eprintln!("skipping: KIMI_API_KEY not set");
            return None;
        },
    };
    let base_url = std::env::var("KIMI_BASE_URL").unwrap_or_else(|_| "https://api.moonshot.cn/v1".to_string());
    ...
    let model = std::env::var("KIMI_MODEL").unwrap_or_else(|_| {
        if base_url.contains("kimi.com/coding") { "kimi-for-coding".to_string() } else { "kimi-k2.5".to_string() }
    });
    Some((api_key, base_url, model))
}

/// Whether this model ID should omit the `thinking` request param
...
fn thinking_enabled(model: &str) -> Map<String, Value> {
    if omit_thinking_param(model) {
        return Map::new();
    }
    // k2.6 uses Preserved Thinking (`keep: "all"`); other K2.x use enabled only.
    let thinking = if model.contains("k2.6") || model.contains("k2-6") {
        json!({ "type": "enabled", "keep": "all" })
    } else {
        json!({ "type": "enabled" })
    };
    json!({ "thinking": thinking }).as_object().cloned().expect("thinking object")
}

fn with_thinking(mut body: Value, model: &str) -> Value {
    let obj = body.as_object_mut().expect("request body object");
    obj.extend(thinking_enabled(model));
    body
}
...
async fn chat_completions(api_key: &str, base_url: &str, body: &Value) -> Result<(reqwest::StatusCode, Value), String> {
...
/// Soft-skip when the key is present but the Coding membership / billing
/// gate rejects the call (common for `api.kimi.com/coding`).
fn skip_if_unavailable(status: reqwest::StatusCode, body: &Value) -> bool {
    if status.as_u16() != 402 {
        return false;
    }
    eprintln!("skipping: Kimi API unavailable (HTTP {status}): {}", serde_json::to_string(body).unwrap_or_default());
    true
}
...
fn reasoning_of(msg: &Value) -> Option<&str> {
    msg.get("reasoning_content").and_then(|v| v.as_str()).filter(|s| !s.is_empty())
}
...
fn keep_latest_reasoning_only(messages: &mut [Value]) {
    let last = messages
        .iter()
        .rposition(|m| m.get("role").and_then(|r| r.as_str()) == Some("assistant") && reasoning_of(m).is_some());
...
async fn kimi_reasoning_plain_multiturn_echo_optional() {
    ...
    let (status1, resp1) = chat_completions(&api_key, &base_url, &turn1_body).await.expect("turn1 request");
    ...
    println!("turn1 ok; reasoning_content present={} len={}", rc1.is_some(), rc1.map(str::len).unwrap_or(0));
    assert!(rc1.is_some(), "expected reasoning_content in thinking mode: {asst1}");

    // Turn 2a: omit reasoning_content.
    let turn2_omit = with_thinking(
        json!({
            "model": model,
            "messages": [
    ...
    let (status_omit, resp_omit) =
        chat_completions(&api_key, &base_url, &turn2_omit).await.expect("turn2 omit request");
    ...
    // Turn 2b: echo reasoning_content.
    let turn2_echo = with_thinking(
        json!({
            "model": model,
            "messages": [
                {"role": "user", "content": "Reply with exactly one word: ping"},
    ...
    let (status_echo, resp_echo) =
        chat_completions(&api_key, &base_url, &turn2_echo).await.expect("turn2 echo request");
...
async fn kimi_reasoning_tool_call_echo_required_or_not() {
    ...
    // Turn 1: ask for a tool call (thinking mode may reject forced tool_choice).
    let turn1_body = with_thinking(
        json!({
            "model": model,
            "messages": [{
    ...
    let (status1, resp1) = chat_completions(&api_key, &base_url, &turn1_body).await.expect("tool turn1 request");
    if skip_if_unavailable(status1, &resp1) {
        return;
    }
    ...
    let asst1 = assistant_message(&resp1);
    let rc1 = reasoning_of(&asst1);
    let tool_calls = asst1.get("tool_calls").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    println!(
    ...
    assert!(rc1.is_some(), "expected reasoning_content alongside tool_calls: {asst1}");
    ...
    // Follow-up WITHOUT reasoning_content (docs claim required / may 400;
    // assert observed status and print it).
    let without_rc = with_thinking(
        json!({
            "model": model,
    ...
    let (status_without, resp_without) =
        chat_completions(&api_key, &base_url, &without_rc).await.expect("tool followup without rc");
    println!(
    ...
    let (status_with, resp_with) =
        chat_completions(&api_key, &base_url, &with_rc).await.expect("tool followup with rc");
    println!("tool followup WITH reasoning_content → {status_with}");
    assert!(status_with.is_success(), "tool followup with reasoning_content must succeed: {status_with} {resp_with}");
    ...
    if status_without.is_success() {
        println!(
            "NOTE: Kimi accepted tool followup WITHOUT reasoning_content \
             (docs say required / may 400; live API currently allows it)."
        );
    } else {
        assert!(
...
async fn kimi_reasoning_tool_call_echo_latest_only() {
    ...
    let (s1, r1) = chat_completions(&api_key, &base_url, &round1).await.expect("round1");
    if skip_if_unavailable(s1, &r1) {
        return;
    }
    ...
    let round1_finish = with_thinking(
        json!({
            "model": model,
    ...
    let (s1b, r1b) = chat_completions(&api_key, &base_url, &round1_finish).await.expect("round1 finish");
    ...
    let (s2, r2) = chat_completions(&api_key, &base_url, &round2).await.expect("round2");
    ...
    assert!(before >= 2, "need at least two historical thinkings to exercise latest-only (got {before})");

    let latest_only = with_thinking(
        json!({
            "model": model,
            "messages": history,
            "tools": [date_tool()],
    ...
    let (status, resp) = chat_completions(&api_key, &base_url, &latest_only).await.expect("latest-only followup");
    println!(
        "tool history with ONLY latest reasoning_content → {status} body={}",
        serde_json::to_string(&resp).unwrap_or_default()
    );
    assert!(status.is_success(), "echoing only the latest reasoning_content should succeed: {status} {resp}");
}

...
use harness::{
    bash_tool_use, first_index, mock_turn, mock_turn_with_usage, read_file_tool_use, run_commands, run_single_task,
    run_single_task_with_setup, sample_token_usage, step_finished_ids, text_block, write_file_tool_use,
};
...
async fn parallel_read_files_both_succeed() {
    let mock = MockClient::new(vec![
        mock_turn(
            vec![read_file_tool_use("read_a", "a.txt"), read_file_tool_use("read_b", "b.txt")],
            StopReason::ToolUse,
        ),
        mock_turn(vec![text_block("Both files read.")], StopReason::EndTurn),
    ]);

    let (updates, _work_dir) = run_single_task_with_setup(mock, "read both", PermissionMode::Auto, |work_dir| {
...
async fn large_non_bash_output_is_persisted() {
    let mock = MockClient::new(vec![
        mock_turn(vec![read_file_tool_use("read_big", "big.txt")], StopReason::ToolUse),
        mock_turn(vec![text_block("Large read handled.")], StopReason::EndTurn),
    ]);
    let (updates, work_dir) = run_single_task_with_setup(mock, "read big", PermissionMode::Auto, |dir| {
...
async fn plan_mode_blocks_write_file() {
    let mock = MockClient::new(vec![
        mock_turn(vec![write_file_tool_use("w1", "blocked.txt", "nope")], StopReason::ToolUse),
    ...
    assert!(!work_dir.join("blocked.txt").exists(), "denied write must not create file");
}

#[tokio::test]
async fn bash_echo_returns_success() {
    let mock = MockClient::new(vec![
        mock_turn(vec![bash_tool_use("bash1", "echo harness-ok")], StopReason::ToolUse),
...
async fn bash_streams_progress_before_step_finished() {
    let command = "printf 'out-1\\n'; sleep 0.1; printf 'err-1\\n' >&2; sleep 0.1; printf 'out-2\\n'";
    let mock = MockClient::new(vec![
        mock_turn(vec![bash_tool_use("bash_stream", command)], StopReason::ToolUse),
    ...
    let progress_text = updates
        ...
        .filter_map(|update| match update {
            AgentUpdate::ToolProgress { tool_id, chunks } if tool_id == "bash_stream" => {
                Some(chunks.iter().map(|chunk| chunk.text.as_str()).collect::<String>())
            },
            _ => None,
        })
        .collect::<String>();
...
async fn read_then_write_chain() {
    let mock = MockClient::new(vec![
        mock_turn(vec![read_file_tool_use("read1", "source.txt")], StopReason::ToolUse),
        mock_turn(vec![write_file_tool_use("write1", "dest.txt", "copied")], StopReason::ToolUse),
        mock_turn(vec![text_block("Chain done.")], StopReason::EndTurn),
    ]);

    let (updates, work_dir) = run_single_task_with_setup(mock, "copy file", PermissionMode::Auto, |dir| {
    ...
    assert_eq!(std::fs::read_to_string(work_dir.join("dest.txt")).unwrap(), "copied");
}

#[tokio::test]
async fn cancel_then_submit_completes_fresh_task() {
    let mock = MockClient::new(vec![
        mock_turn(vec![bash_tool_use("bash_sleep", "sleep 2")], StopReason::ToolUse),
    ...
    let (updates, _work_dir) = run_commands(mock, PermissionMode::Auto, |user_cmd_tx| {
        tokio::spawn(async move {
            user_cmd_tx.send(UserCommand::SubmitTask("slow task".into())).unwrap();
            tokio::time::sleep(Duration::from_millis(100)).await;
            user_cmd_tx.send(UserCommand::Cancel).unwrap();
            user_cmd_tx.send(UserCommand::SubmitTask("fast recovery".into())).unwrap();
            drop(user_cmd_tx);
        })
    })
    .await;
    ...
    let completes = updates.iter().filter(|u| matches!(u, AgentUpdate::TaskComplete(_))).count();
    assert_eq!(completes, 1, "only the recovery task should TaskComplete");
}

#[tokio::test]
async fn submit_task_emits_model_info() {
    let mock = MockClient::new(vec![mock_turn(vec![text_block("With model info.")], StopReason::EndTurn)]);

    let (updates, _work_dir) = run_single_task(mock, "hello", PermissionMode::Auto).await;

    assert!(
        updates.iter().any(|u| matches!(u, AgentUpdate::ModelInfo(_))),
...
async fn tool_turn_persists_to_session_store() {
    ...
    let mock = MockClient::new(vec![
        mock_turn(vec![read_file_tool_use("read1", "data.txt")], StopReason::ToolUse),
    ...
    let driver = tokio::spawn(tact_ui::driver::run_command_loop(agent, user_cmd_rx, work_dir));
    user_cmd_tx.send(UserCommand::SubmitTask("read and persist".into())).unwrap();
    drop(user_cmd_tx);
    driver.await.unwrap();
    ...
    assert!(messages.len() >= 2, "tool turn should persist user + assistant messages, got {}", messages.len());
}
...
async fn read_write_same_file_serializes() {
    ...
    let (updates, work_dir) =
        run_single_task_with_setup(mock, "read then write shared.txt", PermissionMode::Auto, |dir| {
            write_workspace_file(dir, "shared.txt", "original")
        })
        .await;

    let read_done =
        first_index(&updates, |u| matches!(u, AgentUpdate::StepFinished { tool_id: id, .. } if id == "read_shared"));
    let write_done =
        first_index(&updates, |u| matches!(u, AgentUpdate::StepFinished { tool_id: id, .. } if id == "write_shared"));
    assert!(
        read_done.is_some() && write_done.is_some() && read_done < write_done,
        "read must finish before write on same path, got: {updates:?}"
    );
    assert_eq!(std::fs::read_to_string(work_dir.join("shared.txt")).unwrap(), "updated");
}

...
use harness::{bash_tool_use, edit_file_tool_use, mock_turn, run_single_task_with_permission_choice};
...
async fn default_mode_allow_once_runs_edit_file() {
    let mock = MockClient::new(vec![
        mock_turn(vec![edit_file_tool_use("e1", "lib.rs", "fn old()", "fn new()")], StopReason::ToolUse),
        mock_turn(vec![harness::text_block("Edit complete.")], StopReason::EndTurn),
    ]);

    let (updates, work_dir) =
        run_single_task_with_permission_choice(mock, "edit lib.rs", PermissionMode::Default, Some(0), |dir| {
            write_workspace_file(dir, "lib.rs", "fn old() {}")
        })
        .await;
...
async fn default_mode_deny_blocks_edit_file() {
    let mock = MockClient::new(vec![
        mock_turn(vec![edit_file_tool_use("e1", "lib.rs", "fn old()", "fn new()")], StopReason::ToolUse),
        mock_turn(vec![harness::text_block("Edit denied.")], StopReason::EndTurn),
    ]);

    let (updates, work_dir) =
        run_single_task_with_permission_choice(mock, "edit lib.rs", PermissionMode::Default, Some(1), |dir| {
            write_workspace_file(dir, "lib.rs", "fn old() {}")
        })
        .await;
...
async fn high_risk_bash_shell_guard_blocks_after_permission_allow() {
    let mock = MockClient::new(vec![
        mock_turn(vec![bash_tool_use("bash_sudo", "sudo echo ok")], StopReason::ToolUse),
        mock_turn(vec![harness::text_block("Done.")], StopReason::EndTurn),
    ]);

    let (updates, _work_dir) =
        run_single_task_with_permission_choice(mock, "run sudo echo", PermissionMode::Default, Some(0), |_| {}).await;
...
async fn always_allow_skips_second_permission_prompt() {
    let mock = MockClient::new(vec![
        mock_turn(vec![edit_file_tool_use("e1", "a.rs", "v1", "v2")], StopReason::ToolUse),
        mock_turn(vec![edit_file_tool_use("e2", "a.rs", "v2", "v3")], StopReason::ToolUse),
        mock_turn(vec![harness::text_block("Both edits.")], StopReason::EndTurn),
    ]);

    let (updates, work_dir) =
        run_single_task_with_permission_choice(mock, "edit twice", PermissionMode::Default, Some(2), |dir| {
            write_workspace_file(dir, "a.rs", "v1")
        })
        .await;
    ...
    let request_selects = updates
        ...
        .count();
    assert_eq!(request_selects, 1, "always-allow label should appear once, got: {updates:?}");

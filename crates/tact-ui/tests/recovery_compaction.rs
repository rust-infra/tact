...
use harness::{bash_tool_use, read_file_tool_use, run_single_task_with_config, task_completed_with, text_block};
...
fn error_contains(updates: &[AgentUpdate], needle: &str) -> bool {
    updates.iter().any(|update| matches!(update, AgentUpdate::Error(error) if error.to_string().contains(needle)))
}
...
async fn context_limit_triggers_auto_compact() {
    ...
    let mock = MockClient::with_usage(vec![
        (
            ...
            TokenUsageInfo { total: 85_000, ..TokenUsageInfo::default() },
        ),
        (
        ...
        (vec![text_block("Done after compact.")], Some(StopReason::EndTurn), TokenUsageInfo::default()),
    ]);

    let (updates, work_dir) =
        run_single_task_with_config(mock, "read big file", PermissionMode::Auto, tiny_context_config(), |dir| {
            write_workspace_file(dir, "big.txt", &big_content)
        })
        .await;

    assert!(
        updates.iter().any(|u| matches!(u, AgentUpdate::Info(msg) if msg.contains("[auto compact]"))),
        "expected auto compact info, got: {updates:?}"
    );
    assert!(
        updates.iter().any(|u| matches!(u, AgentUpdate::Info(msg) if msg.contains("[transcript saved"))),
    ...
    assert!(
        transcript_dir.read_dir().map(|mut d| d.next().is_some()).unwrap_or(false),
...
async fn end_turn_does_not_compact_until_another_model_call_is_needed() {
    ...
    let (updates, _) =
        run_single_task_with_config(mock, "finish", PermissionMode::Auto, tiny_context_config(), |_| {}).await;

    assert!(task_completed_with(&updates, "Finished near the context limit"));
    assert!(
...
async fn failed_compact_tool_does_not_trigger_manual_compaction() {
    ...
    let mock = MockClient::with_responder(move |request, idx| match idx {
        0 => Ok((vec![invalid_compact.clone()], Some(StopReason::ToolUse), None)),
        _ => {
            ...
            if prompt.contains("Summarize this coding-agent conversation") {
                ...
                Ok((vec![text_block("Continued after rejected compact.")], Some(StopReason::EndTurn), None))
            }
        },
    });

    let (updates, _) =
        run_single_task_with_config(mock, "compact", PermissionMode::Auto, tiny_context_config(), |_| {}).await;

    assert!(task_completed_with(&updates, "Continued after rejected compact"));
    assert!(
...
async fn prompt_too_long_recovery_compacts_and_retries() {
    let mock = MockClient::with_responder(move |request, idx| {
        match idx {
            ...
            1 => Ok((vec![text_block("Compacted summary.")], Some(StopReason::EndTurn), None)),
            ...
            _ => {
                ...
                Ok((vec![text_block("Recovered from long prompt.")], Some(StopReason::EndTurn), None))
            },
        }
    });
    ...
    let (updates, _work_dir) = run_single_task_with_config(mock, "recover", PermissionMode::Auto, config, |_| {}).await;
...
async fn compact_summary_retries_transient_transport_error() {
    let mock = MockClient::with_responder(|_request, idx| match idx {
        0 => Err(LlmError::Other("prompt is too long".to_string())),
        1 => Err(LlmError::Other("service temporarily unavailable".to_string())),
        2 => Ok((vec![text_block("Summary after retry.")], Some(StopReason::EndTurn), None)),
        _ => Ok((vec![text_block("Recovered after summary retry.")], Some(StopReason::EndTurn), None)),
    });
    let mut config = tiny_context_config();
    config.agent.model_context_window = 200_000;
    let (updates, _) = run_single_task_with_config(mock, "recover", PermissionMode::Auto, config, |_| {}).await;

    assert!(
        updates.iter().any(|update| matches!(update, AgentUpdate::Info(message) if message.contains("compact retry")))
    );
    assert!(task_completed_with(&updates, "Recovered after summary retry"));
}
...
async fn compact_summary_rejects_empty_text_response() {
    ...
    assert!(error_contains(&updates, "summary response contained no text"));
    assert!(!updates.iter().any(|update| matches!(update, AgentUpdate::TaskComplete(_))));
}
...
async fn compact_summary_rejects_truncated_response() {
    let mock = MockClient::with_responder(|_request, idx| match idx {
        0 => Err(LlmError::Other("prompt is too long".to_string())),
        _ => Ok((vec![text_block("partial summary")], Some(StopReason::MaxTokens), None)),
    });
    let mut config = tiny_context_config();
    config.agent.model_context_window = 200_000;
    let (updates, _) = run_single_task_with_config(mock, "recover", PermissionMode::Auto, config, |_| {}).await;
...
async fn compact_summary_request_is_window_aware_for_oversized_turn() {
    ...
    let mock = MockClient::with_responder(|request, idx| match idx {
        ...
        1 => {
            ...
            assert!(prompt.chars().count() < 100_000, "summary prompt was not bounded");
            Ok((vec![text_block("Bounded summary.")], Some(StopReason::EndTurn), None))
        },
        _ => Ok((vec![text_block("Recovered from oversized turn.")], Some(StopReason::EndTurn), None)),
    });
    let mut config = tiny_context_config();
    config.agent.model_context_window = 35_000;
    config.agent.max_tokens = 2_000;
    let (updates, _) = run_single_task_with_config(mock, &task, PermissionMode::Auto, config, |_| {}).await;

    assert!(task_completed_with(&updates, "Recovered from oversized turn"));
}
...
async fn max_tokens_with_pending_tools_executes_then_continues() {
    let mock = MockClient::new(vec![
        ...
        (vec![text_block("Continued after max_tokens.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, _work_dir) =
        run_single_task_with_config(mock, "truncated tool", PermissionMode::Auto, tiny_context_config(), |_| {}).await;

    assert!(
        updates.iter().any(|u| matches!(u, AgentUpdate::StepFinished { tool_id: id, .. } if id == "bash1")),
...
async fn max_tokens_with_large_pending_tool_result_compacts_before_continuation() {
    ...
    let mock = MockClient::with_responder(|request, idx| match idx {
        0 => Ok((
            ...
            Some(StopReason::MaxTokens),
            Some(TokenUsageInfo { total: 30_000, ..TokenUsageInfo::default() }),
        )),
        1 => {
            let prompt = serde_json::to_string(&request.messages).unwrap();
            assert!(prompt.contains("Summarize this coding-agent conversation"), "expected compaction request: {prompt}");
            Ok((vec![text_block("Summary before continuation.")], Some(StopReason::EndTurn), None))
        },
        _ => Ok((vec![text_block("Continued after compact.")], Some(StopReason::EndTurn), None)),
    });

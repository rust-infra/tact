...
use harness::{
    apply_patch_tool_use, ask_user_tool_use, ask_user_tool_use_ex, background_run_tool_use, check_background_tool_use,
    cron_create_tool_use, cron_list_tool_use, load_skill_tool_use, read_inbox_tool_use,
    run_single_task_with_permission_choice, run_single_task_with_setup, save_memory_tool_use, search_code_tool_use,
    send_message_tool_use, spawn_teammate_tool_use, task_completed_with, text_block, worktree_create_tool_use,
...
async fn ask_user_tool_returns_question() {
    let mock = MockClient::new(vec![
        (vec![ask_user_tool_use("ask1", "What is your name?", None)], Some(StopReason::ToolUse)),
        (vec![text_block("Done.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, _work_dir) = run_single_task_with_setup(mock, "ask user", PermissionMode::Auto, |_| {}).await;
...
async fn ask_user_with_options_uses_select_popup() {
    let mock = MockClient::new(vec![
        (vec![ask_user_tool_use("ask1", "Pick one", Some(&["alpha", "beta"]))], Some(StopReason::ToolUse)),
    ...
    let (updates, _work_dir) =
        run_single_task_with_permission_choice(mock, "ask user options", PermissionMode::Auto, Some(1), |_| {}).await;
...
async fn ask_user_multi_select_uses_multi_popup() {
    let mock = MockClient::new(vec![
        (vec![ask_user_tool_use_ex("ask1", "Pick many", Some(&["a", "b", "c"]), true)], Some(StopReason::ToolUse)),
        (vec![text_block("Multi done.")], Some(StopReason::EndTurn)),
    ]);

    // Harness maps choice 0 → multi selection [0] → "a".
    let (updates, _work_dir) =
        run_single_task_with_permission_choice(mock, "ask user multi", PermissionMode::Auto, Some(0), |_| {}).await;
...
async fn save_memory_persists_file() {
    let mock = MockClient::new(vec![
        (
            vec![save_memory_tool_use("mem1", "test_preference", "user", "tab preference", "I prefer tabs.")],
            Some(StopReason::ToolUse),
        ),
        (vec![text_block("Memory saved.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, work_dir) = run_single_task_with_setup(mock, "save memory", PermissionMode::Auto, |_| {}).await;
    ...
    let memory_file = work_dir.join(".claude").join("memory").join("test_preference.md");
...
async fn load_skill_reads_skill_file() {
    let mock = MockClient::new(vec![
        (vec![load_skill_tool_use("skill1", "rust_style")], Some(StopReason::ToolUse)),
        (vec![text_block("Skill loaded.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, _work_dir) = run_single_task_with_setup(mock, "load skill", PermissionMode::Auto, |dir| {
...
async fn teammate_spawn_send_read_inbox() {
    let mock = MockClient::new(vec![
        (vec![spawn_teammate_tool_use("spawn1", "reviewer", "Code reviewer")], Some(StopReason::ToolUse)),
        (vec![send_message_tool_use("msg1", "user", "reviewer", "Please review.")], Some(StopReason::ToolUse)),
        (vec![read_inbox_tool_use("inbox1", "reviewer")], Some(StopReason::ToolUse)),
        (vec![text_block("Teammate flow done.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, _work_dir) = run_single_task_with_setup(mock, "teammate flow", PermissionMode::Auto, |_| {}).await;
...
async fn cron_create_list_delete() {
    let mock = MockClient::new(vec![
        (vec![cron_create_tool_use("cron1", "0 0 * * *", "Daily summary.")], Some(StopReason::ToolUse)),
        (vec![cron_list_tool_use("cron2")], Some(StopReason::ToolUse)),
        (vec![text_block("Cron flow done.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, _work_dir) = run_single_task_with_setup(mock, "cron flow", PermissionMode::Auto, |_| {}).await;
...
async fn background_run_and_check() {
    let mock = MockClient::new(vec![
        (vec![background_run_tool_use("bg1", "sleep 0.1 && echo bg-ok")], Some(StopReason::ToolUse)),
        (vec![check_background_tool_use("bg2", None)], Some(StopReason::ToolUse)),
        (vec![text_block("Background done.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, _work_dir) = run_single_task_with_setup(mock, "background flow", PermissionMode::Auto, |_| {}).await;
    ...
    assert!(
        updates.iter().any(|u| matches!(u, AgentUpdate::StepFinished { tool_id: id, .. } if id == "bg2")),
...
async fn search_code_finds_matches() {
    let mock = MockClient::new(vec![
        (vec![search_code_tool_use("search1", "fn answer", Some("src"))], Some(StopReason::ToolUse)),
        (vec![text_block("Search done.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, _work_dir) = run_single_task_with_setup(mock, "search code", PermissionMode::Auto, |dir| {
...
"#;
    ...
    let mock = MockClient::new(vec![
        (vec![apply_patch_tool_use("patch1", patch)], Some(StopReason::ToolUse)),
        (vec![text_block("Patch applied.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, work_dir) = run_single_task_with_setup(mock, "apply patch", PermissionMode::Auto, |dir| {
...
async fn worktree_create_lists_and_shows_status() {
    let mock = MockClient::new(vec![
        (vec![worktree_create_tool_use("wt1", "feature-x", None)], Some(StopReason::ToolUse)),
        (vec![worktree_list_tool_use("wt2")], Some(StopReason::ToolUse)),
        (vec![worktree_status_tool_use("wt3", "feature-x")], Some(StopReason::ToolUse)),
        (vec![text_block("Worktree done.")], Some(StopReason::EndTurn)),
    ]);

    let (updates, _work_dir) = run_single_task_with_setup(mock, "worktree flow", PermissionMode::Auto, |dir| {

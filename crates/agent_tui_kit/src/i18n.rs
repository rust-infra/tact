...
impl Messages {
    ...
    fn english() -> Self {
        Self {
            ...
            mcp_usage: "Usage: /mcp auth <server> (or /mcp login <server>) — authorize a remote (OAuth) MCP server. Manage servers from the CLI: `tact-ui mcp list|get|add|remove|login|logout`.",
    ...
    fn chinese() -> Self {
        Self {
            ...
            mcp_usage: "用法：/mcp auth <服务器名>（或 /mcp login <服务器名>）—— 为远程（OAuth）MCP 服务器授权。用 CLI 管理服务器：`tact-ui mcp list|get|add|remove|login|logout`。",

//! History panel (task history list).
...
use crate::{
    render::{ctx::RenderCtx, util::truncate_chars_with_ellipsis},
    widgets::popup_widget::PopupWidget,
};

pub fn render_history_panel(frame: &mut Frame, area: Rect, ctx: &RenderCtx) {
    let count = ctx.task_history.len();
    ...
    let widget = PopupWidget::default()
        ...
        .with_title(ctx.messages.history_title);

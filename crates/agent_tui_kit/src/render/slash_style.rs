//! Syntax highlighting for `/skill-name` [args] in the input box and user log
...
pub fn skill_name_set<'a>(skills: &'a [SkillEntry], builtins: &HashSet<&str>) -> HashSet<&'a str> {
    skills
        .iter()
        .map(|s| s.name.as_str())
...
mod tests {
    ...
    fn skill_name_set_excludes_builtin_names() {
        ...
        let builtins: HashSet<&str> = ["help"].into_iter().collect();
        let names = skill_name_set(&skills, &builtins);
